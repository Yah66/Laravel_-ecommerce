    ...
    <div class="d-flex flex-column-fluid">
        ...
        <div class=" container ">
            ...
            <div class="alert-text">
                @if ($addVendor)
                    @include('livewire.admin.vendor.createVendor')
                @endif
                @if ($updateVendor)
                    @include('livewire.admin.vendor.updateVendor')
                @endif
                @include('livewire.admin.vendor.deleteVendor')
            ...
            <div class="card card-custom">
                <div class="card-header flex-wrap py-5">
                    <div class="card-title">
                        <h3 class="card-label">
                            {{ __('messages.vendor') }}
                            <span class="text-muted pt-2 font-size-sm d-block">{{ __('messages.all_vendors') }}</span>
                        </h3>
                    </div>
                    <div class="card-toolbar">
                        ...
                        <!--begin::Button-->
                        <a wire:click="addVendor()"class="btn btn-primary font-weight-bolder" data-toggle="modal"
                            data-target="#Vendor_form">
                    ...
                    <div id="kt_datatable_wrapper" class="dataTables_wrapper dt-bootstrap4 no-footer">
                        ...
                        <div class="row py-3">
                            <div class="col-sm-12">
                                    ...
                                    <thead>
                                        <tr role="row">
                                            ...
                                            <th class="sorting" tabindex="0" aria-controls="kt_datatable"
                                                ...
                                                style="width: 47px;">{{ __('messages.phone') }}</th>

                                            <th class="sorting" tabindex="0" aria-controls="kt_datatable"
                                                rowspan="1" colspan="1"
                                                aria-label="Ship Date: activate to sort column ascending"
                                                style="width: 47px;">{{ __('messages.category_id') }}</th>
                                    ...
                                    <tbody>
                                        @foreach ($data as $d)
                                            <tr class="odd">
                                                ...
                                                <td>{{ $d->name }}</td>
                                                <td><img src="{{ asset('storage/' . $d->logo) }}" alt="image"
                                                        height="65px" width="65px">
                                                </td>
                                                <td>{{ $d->mobile }}</td>
                                                {{-- <td>{{ $d->category->name->en }}</td> --}}
                                ...
                                data-target="#updateVendor" class="btn btn-sm btn-clean btn-icon"
                                title="Edit details">
                                <i class="la la-edit"></i> </a>
                            <a title="Delete" wire:click="$set('vendorIdToDelete', {{ $d->id }})"
                                data-toggle="modal" data-target="#deleteVendor" class="btn btn-sm btn-clean btn-icon"
                                title="Delete">
